...
class CairoTransportationData:
    ...
    def _add_road_edge(
        ...
        for period in self.PERIODS:
            ...
            attrs[f"{period}_travel_time"] = round(
                distance * (1 + period_ratio) * condition_penalty, 3
            )
    ...
    def _connect_isolated_facilities(self) -> None:
        ...
        for facility in facilities:
            ...
            nearest = min(
                neighborhoods,
                key=lambda node: self._approx_distance_km(facility, node)
            )
    ...
    def find_traffic(self, u: str, v: str) -> Dict[str, float]:
        ...
        for pattern in self.traffic_patterns:
            if pattern.get("road") in {key1, key2}:
                return {
                    period: float(pattern.get(period, 0))
                    for period in self.PERIODS
                }
